...
def test_sshfs_mount_hub(image, swarm, network, make_service):
    ...
    with requests.Session() as s:
        ...
        mount_resp = s.post(JHUB_URL + "/hub/mount", headers=headers)
...
access_hub_config = join(dirname(realpath(__file__)), 'configs',
                         'access_jupyterhub_config.py')
access_hub_service = {'image': 'nielsbohr/jupyterhub:devel', 'name': HUB_SERVICE_NAME,
                      'mounts': [
                          ':'.join(['/var/run/docker.sock', '/var/run/docker.sock',
                                    'rw']),
...
# @pytest.mark.parametrize('swarm', [swarm_config], indirect=['swarm'])
# @pytest.mark.parametrize('network', [network_config], indirect=['network'])
# def test_access_hub(image, swarm, network, make_service):
#     """Test that logging in as a new user creates a new docker service."""
#     make_service(access_hub_service)
#     client = docker.from_env()
#     # jupyterhub service should be running at this point
#     services_before_spawn = client.services.list()
#
...
#         }
#         spawn_resp = s.post(JHUB_URL + "/hub/spawn", data=payload)
#         # Failed spawn cause of no access
#         assert spawn_resp.status_code == 500
#
#         # logout
#         logout_resp = s.post(JHUB_URL + '/hub/logout')
#         logout_resp.status_code == 200
#
#         # login with allowed user
#         login_response = s.post(JHUB_URL + "/hub/login?next=",
#                                 data={"username": privileged-user,
#                                       "password": "just magnets"})
#         assert login_response.status_code == 200
#
#
